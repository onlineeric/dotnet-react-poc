...
- `Program.cs` — single-file server: CORS, static file serving, API endpoints, SPA fallback routing, and data models
- `client/dist/` — build output directory for the React SPA, served as static files via `PhysicalFileProvider`
- `data/products.json` — JSON file used as the data store, read/written by API endpoints
